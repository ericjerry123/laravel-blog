<x-layouts>
    <div class="max-w-4xl mx-auto px-4 py-8">
        ...
        <form action="{{ route('posts.update', $post->id) }}" method="POST" class="bg-base-100 p-6 rounded-lg shadow-sm">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <textarea name="content" id="content" rows="10" 
                          class="textarea textarea-bordered w-full" required>{{ old('content', $post->content) }}</textarea>
                @error('content')
                    <p class="text-error text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-end gap-2">
                <a href="{{ route('posts.show', $post->id) }}" class="btn btn-ghost">取消</a>
